...
This is where the abstract begins...
...
Biologists developed several types of representation to illustrate molecular structures, and are supported by mainstream visualization packages such as VMD~\cite{humphrey1996vmd} or PyMol~\cite{delano2002pymol}.
...
CellBlender[] is a software conceived as a plug-in for the 3D animation package Blender, which allows model design and visualization of particle-based models computed with MCell~\cite{kerr2008fast}.
...
The approach which we designed directly follows-up the work of Lampe et al.[] and focuses on reducing the GPU driver-overhead, and balancing geometry processing using the GPU programmable rendering pipeline.
...
This concept was applied by Lampe et al.[] who used the geometry shader stage to instantiate the atomic structure of entire amino acids, also called residues, directly from the GPU rasterization pipeline.
...
Similarly to Lampe et al.[], only one initial vertex shader thread is required to draw an entire molecule, but the maximum number of atoms which is supported is up to two orders of magnitude greater with our approach.
...
An utterly important component of living organisms are fiber and polymer structures, such as DNA, RNA, \textbf{or ... ask ludo maybe}.
Similarly to protein data, a challenge when rendering dynamic fibre structures is memory usage and bandwidth.
Fibres such as DNA, are composed of small monomers called nucleic acids, also known as bases, and which are made out of a few dozen of atoms.
One approach to render fibre structures would be to store the position, rotation, and type, of each nucleic acids and to render the bases with the same approach as for protein data and described above.
...
Indeed, the entire genome of E.Coli, for instance, is composed of \textbf{xxx ask ludo} individual base pairs, which represents \textbf{xxx} GB of data.
...
Therefore an accurate depiction of living organisms often results in a very dense arrangement of macromolecules as shown in \textbf{Figure X[]}.
...
A single bar comprises three distinct regions that indicate the visibility properties for a given species as shown in Figure[].
...
Falk et al.[] designed a visualization tool to playback the particle trajectories resulting from the simulation in 3D space.
