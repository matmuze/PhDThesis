...
Structural biology is concerned by the structure of biological macromolecules, such as proteins or DNA, for example, and the relationship between molecular structure and function.
...
Acquired atomic structures are often stored in digital files and shared via public data bases, such as the Protein Data Bank[], to facilitate collaboration among peers.
...
Biologists developed several types of representation to illustrate molecular structures, and are supported by mainstream visualization packages such as VMD[] or PyMol[].
The simplest representation is the sticks model, where each bonds are represented as a line, and color coding is used to indicate the atom type at the line extremities or joints.
...
In scientific illustrations, the shape of a protein is an important aspect to convey as it is tightly related to its function, and therefore the surfaces or VdW representations are often preferred, because they communicate shape information efficiently.
...
However, the overwhelmingly large number of elements that may compose these mesoscale structures begin to truly challenge animation packages, that were not design with such constraints in mind.
...
Indeed, to render highly detailed meshes require an overwhelmingly large number of polygons which can stress the rendering pipeline and video memory usage.
...
As a result, to render a molecule which comprise 1000 atoms, for example, only 4000 vertices, 4 vertices per atom to form the camera facing quads, would then be needed in order to draw very detailed and smooth ensembles of spheres, while polygon meshes would require a number of vertices up to one or two orders of magnitude higher.
...
Shortly afterwards, Lampe et al. [8] extended the billboard technique by leveraging the programmable GPU rendering pipeline to reduce memory bandwidth usage and GPU driver overhead. 
...
Alternatively, Grottel at al.[] proposed to improve the rendering speed of large particle-based datasets by implementing occlusion culling to discard hidden particle chunks from the rendering pipeline, based on the depth information obtained in the previous frame.
...
Lindow et al. [11], subsequently presented a novel approach which relies on ray-casting instead.
...
A shortcoming of the volumetric approach presented by Lindow et al.[], is that for each rendered macro-molecule, additional expensive ray-traversal routines are required during the per-fragment processing, which may unbalance parallel thread execution and cause considerable bottlenecks, especially with dense scenes composed of small individual macromolecules.
We opted for an impostor-based method for the design of our rendering pipeline, which requires more individual per-fragment thread execution per macro-molecule, but is far more balanced and suitable for paralleled processing, and also do not require expensive volume sampling operation upon rendering. 
The approach which we designed directly follows-up the work of De Lampe et al.[] and focuses on reducing the GPU driver-overhead, and balancing geometry processing using the GPU programmable rendering pipeline.
...
While the geometry shader is only designed to inject a few dozens of triangle maximum, the tessellation shader is able to efficiently inject thousands of triangle by dispatching the work on multiple threads.
...
Similarly to De Lampe et al.[], only one initial vertex shader thread is required to draw an entire molecule, but the maximum number of atoms which is supported is up to two orders of magnitude greater with our approach.
...
Grottel et al.[] presented an efficient rendering pipeline for large particles-based datasets, using HOQ to discard large chunks of the dataset hidden in the current viewport.
...
Based on the pixel coverage and position of the squares, they perform a series of look-up at a specific resolution in order to minimize the number of texture accesses, which tend to slow the computation.
...
One approach to render fibre structures would be to store the position, rotation, and type, of each nucleic acids and to render the bases with the same approach used for protein data and described above.
...
The rendering pipeline which we developed for the fibres relies on the tessellation shader to dynamically inject spheres impostors that compose each fibre segments, similarly to protein data.
